import React from 'react';
import './PurpleWaveBottom.css';

const PurpleWaveBottom = (props) => {
  return (
    <div className="wave-container-bottom">
      <div className="purple-wave-bottom">
          <svg width="1440" height="3357" viewBox="0 0 1440 3357" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M-1.99997 169.468C-1.99997 169.468 235.919 -150.738 716.756 89.149C1197.59 329.036 1101.13 96.7581 1442.51 190.181L1443 3334C1443 3334 1059.19 3054.5 994 3129C556.5 3629 529.5 2975.5 353 3241.5C176.5 3507.5 0.500031 3227 0.500031 3227L-1.99997 169.468Z" fill="#E3E5FC"/>
          </svg>
      </div>
    </div>
  )
};

export default PurpleWaveBottom;